"""Train the Recurrent Transformer with Attention Residuals (no-world-knowledge)."""
...
MIN_LR = 1e-4
WARMUP_STEPS = 200
MAX_STEPS = int(os.environ.get("TRAIN_STEPS", "4000"))
RESUME_CKPT = os.environ.get("RESUME_CKPT", "")   # e.g. "final.pt" or "best.pt" to continue training
LOG_EVERY = 25
EVAL_EVERY = 250
SAVE_EVERY = 500
SEED = 42
...
def main():
    ...
    if RESUME_CKPT:
        resume_path = os.path.join(MODEL_DIR, RESUME_CKPT)
        ...
        print(f"Resumed from {resume_path} at step {step} (best_val {best_val:.4f})")
        ...
        if step % EVAL_EVERY == 0 and step > 0:
            ...
            if val_loss < best_val:
                ...
                torch.save({"model": model.state_dict(), "cfg": cfg, "step": step,
                            "optimizer": optimizer.state_dict(), "best_val": best_val}, ckpt)
